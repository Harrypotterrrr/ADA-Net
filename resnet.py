...
from meta_layer import MetaLinear, MetaConv2d, MetaConvTranspose2d, MetaBatchNorm2d, MetaSequential
...
class BasicBlock(nn.Module):
    ...
    def meta_forward(self, x, inner_lr):
        out = F.relu(self.bn1.meta_forward(self.conv1.meta_forward(x, inner_lr), inner_lr))
        out = self.bn2.meta_forward(self.conv2.meta_forward(out, inner_lr), inner_lr)
        out += self.shortcut.meta_forward(x, inner_lr)
        out = F.relu(out)
        return out
...
class ResNet(nn.Module):
    ...
    def meta_forward(self, x, inner_lr):
        out = F.relu(self.bn1.meta_forward(self.conv1.meta_forward(x, inner_lr), inner_lr))
        out = self.layer1.meta_forward(out, inner_lr)
        out = self.layer2.meta_forward(out, inner_lr)
        out = self.layer3.meta_forward(out, inner_lr)
        out = F.avg_pool2d(out, out.size()[3])
        out = out.view(out.size(0), -1)
        out = self.linear.meta_forward(out, inner_lr)
        return out
